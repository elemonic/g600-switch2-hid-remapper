...
試験時は、Switch 2本体下部のUSB-C端子へ非純正のドック相当機器を接続し、そのUSB-A端子とFeatherのUSB-C端子をUSB-A―USB-Cケーブルで接続しました。ドック相当機器とFeatherを直接つないだ構成と、間にUSBハブを挟んだ構成を確認しています。純正Nintendo Switch 2ドックと、本体上部USB-C端子への直接接続は未確認です。
